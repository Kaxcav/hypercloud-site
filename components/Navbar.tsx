...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200/60 bg-white/85 backdrop-blur-md">
      <div className="container-shell flex h-24 items-center gap-4">
        ...
        <div className="ml-auto hidden items-center gap-3 lg:flex">
          ...
          <Link
            ...
            className="rounded-md bg-brand-gradient px-5 py-3 text-sm font-semibold text-white shadow-brand transition hover:opacity-95"
          >
          ...
          <div className="container-shell flex flex-col gap-2 py-4">
            {links.map((link) => (
              <Link
                ...
                className="rounded-xl px-4 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-50 hover:text-brand-700"
              >
                {link.label}
              </Link>
            ))}
            <div className="mt-2 flex flex-col gap-3 border-t border-slate-200/60 pt-4">
              <Link
                href="/portal-do-cliente"
                onClick={() => setOpen(false)}
                className="inline-flex items-center justify-center gap-2 rounded-md border border-slate-200 px-4 py-3 text-sm font-medium text-slate-700 transition hover:border-brand-200 hover:text-brand-600"
              >
                <Lock className="h-4 w-4" />
                Portal do Cliente
              </Link>
